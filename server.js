...
const WEBHOOK_SECRET = process.env.WEBHOOK_SECRET || "ccd-process-capture-2026";
...
app.post("/api/submit", async (req, res) => {
  ...
  try {
    ...
    // Send to Power Automate
    const payload = {
      webhookSecret: WEBHOOK_SECRET, // For validation in Power Automate
...

{
  ...
  "timeline": "string - how long it typically takes",
  "completion": "string - what happens when complete",
  "steps": ["array of critical steps in order - simple strings, not objects"],
  "roles": {
    "responsible": ["who does the work"],
    "accountable": ["who owns the outcome"],
    "consulted": ["who provides input"],
...
app.listen(PORT, () => {
  ...
  console.log(
    `Webhook Secret: ${WEBHOOK_SECRET ? "[configured]" : "[using default]"}`,
  );
  console.log(
